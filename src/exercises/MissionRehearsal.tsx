    ...
    <div className="flex flex-col gap-6 p-4">
      ...
      <div className="flex flex-col gap-3">
        {mission.lines.map((line, i) => (
          <div
            key={i}
            className="flex items-start gap-3 rounded-md bg-surface p-4 ring-1 ring-black/5"
          >
      ...
      {mission.realWorld_uk.length > 0 && (
        <div className="flex flex-col gap-2">
          ...
          {mission.realWorld_uk.map((item, i) => (
            <label
              key={i}
              className="flex items-start gap-3 min-h-tap rounded-md bg-surface px-4 py-3 ring-1 ring-black/5 cursor-pointer"
            >
              <input
                type="checkbox"
                checked={checked[i]}
